...
randomization <- read.csv('../data/clinical/randomization.csv', stringsAsFactors = FALSE) #from Judy (STOPPD RA); 126 randomized, all patients
log <- read.csv('../data/clinical/master_log.csv', fileEncoding="latin1", na.strings=c(""," ","NA", "N/A", "xx"), stringsAsFactors = FALSE) #from Judy (STOPPD RA)
...
df <- df %>% select(STUDYID, age, sex, randomization, MR_exclusion, first_timepoint, second_timepoint, third_timepoint)
